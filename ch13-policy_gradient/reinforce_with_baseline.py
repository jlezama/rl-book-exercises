...
def compute_grad(theta,s,a):
    ...
    grad = x(s,a) - np.dot(pi[not_a], x(s,not_a))
...
################################################################################
def REINFORCE(theta, w, gamma = 1.0, alpha_theta=2**-9, alpha_w=2**-6):
    EPISODES= 200
    ...
    for ep in range(EPISODES):
        ...
        for t in range(len(ep_s)):
           ...
           w += alpha_w * (gamma**t) * delta_t
           
           theta += alpha_theta * (gamma**t) * delta_t  * compute_grad(theta, ep_s[t],ep_a[t])
...
for i in range(RUNS):
    ...
    theta, G_0s_t = REINFORCE(theta,w)
